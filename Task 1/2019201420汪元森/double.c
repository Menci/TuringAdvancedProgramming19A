...
const int BIT_WIDTH = 31 , BIT_X_LEN = 7 ;
const unsigned long long BIT_X_MAX = 0x80000000 ;
const int exp_max = 2047 , E_MIN = -1022 , E_MAX = 1023 ;

const int SIZE_OF_ULL = 8 ;

const char __data_oo[8] = { 0 , 0 , 0 , 0 , 0 , 0 , 0xF0 , 0x7F } ;
const char __data_minus_oo[8] = { 0 , 0 , 0 , 0 , 0 , 0 , 0xF0 , 0xFF } ;
const char __data_NAN[8] = { 0 , 0 , 0 , 0 , 0 , 0 , 0xF8 , 0xFF } ;
const char __data_zero[8] = { 0 , 0 , 0 , 0 , 0 , 0 , 0 , 0 } ;
int search_block[200] ;
...
int A_bigger_B( md A , md B ){/*{{{*/
	...
	static int A_exp , B_exp , cmp , A_fir , B_fir , pt ;
...
md make_md_bit_x( const bool sgn , uint exp , bit_x *frac ){
	...
	ws = 0 ;
	for( pt = 0 ; ws + CHAR_BIT <= FRAC_BIT ; pt ++ , ws += CHAR_BIT )
	...
	for( ; ws + CHAR_BIT <= EXP_BIT ; pt ++ , ws += CHAR_BIT )
...
md bit_x_to_myDouble( bool sgn , int e , bit_x *ori_x , int aclen ){
	...
	static bit_x x , frac_use , frac_round ;
	...
	// get e
	bit_x_LmoveWIDTH( &x ) , aclen += BIT_WIDTH , flo += BIT_WIDTH ;
	...
	frac_round = extract_bit_x( &x , 0 , aclen - FRAC_BIT - 2 ) ;
		
	if( extract_bit_x_byte( &x , aclen - FRAC_BIT - 1 ) ){
		if( bit_x_check_zero( &frac_round ) ){
...
md Minus( md A , md B ){ /*{{{*/
	...
	A_sgn = get_sgn( &A ) , B_sgn = get_sgn( &B ) , sgn = A_sgn ^ B_sgn ;
	if( sgn ){
...
md Multiply( md A , md B ){/*{{{*/
	...
	static int A_e , B_e , C_e , limit , i , j ;
...
md Divide( md A , md B ){/*{{{*/
	...
	rt = bit_x_to_myDouble( sgn , C_e , &tmp , 1.5 * FRAC_BIT ) ;
	return rt ;

}/*}}}*/

void init(){/*{{{*/
	for( int i = 3 * FRAC_BIT ; i ; i -- ) search_block[i] = i / BIT_WIDTH ;
...
uint64_t solve( uint64_t a_ , uint64_t b_ , char opt ){
	static bool inited = false ;
	if( !inited ) init() , inited = true ;

	md A , B , my_ans ;
	uint64_t rt = 0 ;
	double a , b , std_ans , ans ;

	A = *(md*)&a_ , B = *(md*)&b_ ;
	a = *(double*)&a_ , b = *(double*)&b_ ;

	switch( opt ){
		case '+' : /*puts( "option : addition" ) , */my_ans = Add( A , B ) , std_ans = a + b ; break ;
		case '-' : /*puts( "option : subtraction" ) , */my_ans = Minus( A , B ) , std_ans = a - b ; break ;
		case '*' : /*puts( "option : multiplication" ) , */my_ans = Multiply( A , B ) , std_ans = a * b ; break ;
		case '/' : /*puts( "option : division" ) , */my_ans = Divide( A , B ) , std_ans = a / b ; break ;
	}

//	out( my_ans ) , out( *(md*)&std_ans ) ;
...
int main(){
	uint64_t a = solve( 0xbfd0a7e551ca4104ull , 0x4305b7dd88c60eaaull , '+' ) ;
	printf( "%f" , *(double*)&a ) ;
}
