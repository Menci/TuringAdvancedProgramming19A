...
void myHeapSort(void *base, size_t nmemb, int size,
				int (*compar)(const void *, const void *), void *swapBuffer) {
	if (nmemb <= 1) return;
...
void myInsertionSort(void *base, size_t nmemb, size_t size,
	...
	for (void *i = base + size; i < rlim; i = i + size) {
		...
		if (compar(base, tmpBuf) > 0) {
			for (; j >= base; j -= size) COPY(j + size, j, size);
			COPY(base, tmpBuf, size);
		} else {
			for (; compar(j, tmpBuf) > 0; j -= size) COPY(j + size, j, size);
...
void mergeSeq(void *base, size_t nmemb, size_t size,
			  int (*compar)(const void *, const void *)) {
	...
	for (int i = 0; i < MY_THREAD; ++i) {
		if (i < MY_THREAD - 1)
					 ...
					 (nmemb - (nmemb / MY_THREAD) * (MY_THREAD - 1)) * size;
	}
	void *curElem = malloc(size);
	for (int i = 0; i < nmemb; ++i) {
		int tidx = -1;
		for (int j = 0; j < MY_THREAD; ++j)
			if (begin[j] < end[j]) {
				if (~tidx) {
