...
#define check(a, b)                            \
	if (exp(a) == 2047 && frac(a)) return (ul)a; \
...
void Binarycout(ull n) { 
	for (int i=63;i>=0;i--) 
	{ 
		printf("%u",(unsigned int)((n>>i)&1)); 
		if(i==63||i==52) printf(" ");
	} 
	printf("\n");
} 
void Binarycout1(ull n) { 
	for (int i=108;i>=0;i--) 
	{ 
		printf("%u",(unsigned int)((n>>i)&1)); 
//		if(i==63||i==52) printf(" ");
...
ul dev(ul a, ul b) {
	...
	if (exp(a))
		...
		while (!(fraca & (1ull << 52))) fraca <<= 1, exp--;
	}
	if (exp(b))
		fracb += (1ull << 52);
	else {
