...
char regName[MAX_REG + 1][BUFFER_SIZE] = {"%rdi", "%rsi", "%rbx", "%r8",
                                          "%r9",  "%r10", "%rax"};
char oper[MAX_L + 1], t1[BUFFER_SIZE], t2[BUFFER_SIZE], t3[BUFFER_SIZE],
    t4[BUFFER_SIZE];
int max_reg, max_mem = 0, forb = -1;
int sreg[MAX_REG], loc[MAX_L + 5], sram[MAX_L * 2], num1[MAX_L + 5],
    num2[MAX_L + 5], st[MAX_L + 5], typ[MAX_L + 5];
int las[MAX_L + 5];
...
void printExpr(int lv, char op, int rv, char ltyp, char rtyp) {
    char strl[BUFFER_SIZE], strr[BUFFER_SIZE];
    if (ltyp == 'r')
        sprintf(strl, "%s", regName[lv]);
    else if (ltyp == 'p')
        getMemAddr(strl, lv);
    if (rtyp == 'r')
        sprintf(strr, "%s", regName[rv]);
    else if (rtyp == 'p')
        getMemAddr(strr, rv);
    else
        sprintf(strr, "$%d", rv);
...
void xorResult(int x) { printf("xorq %s, %%r11\n", regName[x]); }
void printFileHead() {
    printf("subq $%d, %%rsp\n", STACK_SPACE);
    printf("movq $0, %%r11\n");
}
...
int main() {
    ...
    for (int i = 1; i <= MAX_L; ++i) {
        ...
        if (t2[0] == 'a')
            s1 = atoi(t2 + 1) + MAX_L - 1, las[s1] = i;
        else
            s1 = atoi(t2 + 1), las[s1] = i;
        if (t4[0] == 'a')
            s2 = atoi(t4 + 1) + MAX_L - 1, fl = 1, las[s2] = i;
        else if (t4[0] == 'v')
            s2 = atoi(t4 + 1), fl = 1, las[s2] = i;
        else
            s2 = atoi(t4), fl = 2;
    ...
    for (int i = 1; i <= MAX_L; ++i) {
        if (typ[i] == 1) {
            ...
            if (st[p1] == 1) {
                if (las[p1] == i) {
                    printExpr(loc[p1], oper[i], loc[p2], 'r',
                              st[p2] == 1 ? 'r' : 'p');
                    xorResult(loc[p1]);
                    ...
                    if (oper[i] == '/') {
                        ...
                        printExpr(oldr, oper[i], loc[p2], 'r',
                                  st[p2] == 1 ? 'r' : 'p');
                        xorResult(oldr);
                ...
                if (oper[i] == '/') {
                    ...
                    printExpr(newr, oper[i], loc[p2], 'r',
                              st[p2] == 1 ? 'r' : 'p');
                    xorResult(newr);
                    sreg[newr] = i, loc[i] = newr, st[i] = 1;
                }
