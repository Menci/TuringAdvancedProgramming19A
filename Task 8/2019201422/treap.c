...
static void update(tp_node *now_node)
{
...
{
	tp_node *now_node = (tp_node*)malloc(sizeof(tp_node));
...
static void free_node(tp_node* now_node)
{
	free(now_node->data);
	free((void*)now_node);
}

static void rotate(tp_node **now_node, int k)
{
	tp_node *v = (*now_node)->son[k^1];
...
static void insert(tp_node **now_node,const void *key, cmp_pointer comp, int width)
{
	if((*now_node) == NULL)
	{
		(*now_node) = new_node();
...
static bool del(tp_node **now_node, const void *key, cmp_pointer comp)
{
	if((*now_node)==NULL)
		return false;
	if(!comp(key, (*now_node)->data))
...
static void* find_key(tp_node *now_node, int rnk, cmp_pointer comp)
{
	if(now_node->siz < rnk)
		return NULL;
	while(1)
...
static int find_rnk(tp_node *now_node, const void *key, cmp_pointer comp)
{
	int rnk = 0;
	while(now_node != NULL)
	{
...
static void* find(tp_node *now_node, const void* key, cmp_pointer comp)
{
	void* pre = NULL;
	while(now_node != NULL)
	{
...
static void* find_nxt(tp_node *now_node, const void* key, cmp_pointer comp)
{
	void* nxt = NULL;
	while(now_node != NULL)
	{
...
static void free_tree(tp_node *now_node)
{
	if(now_node==NULL)
		return;
	free(now_node->data);
...
void* Treap_find(Treap *my_treap, void* key)
{
	return find(my_treap->root, key, my_treap->comp);
}
